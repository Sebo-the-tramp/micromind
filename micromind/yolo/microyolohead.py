...
class Microhead(nn.Module):
    def __init__(
        ...
        layer13.i, layer13.f, layer13.type, layer13.n = (
            13 + (2 if deeper_head else 0) + (-1 if no_SPPF else 0),
            -1,
            "torch.nn.modules.upsampling.Upsample",
            1,
        )
        ...
        layer14.i, layer14.f, layer14.type, layer14.n = (
            14 + (2 if deeper_head else 0) + (-1 if no_SPPF else 0),
        ...
        layer15.i, layer15.f, layer15.type, layer15.n = (
            15 + (2 if deeper_head else 0) + (-1 if no_SPPF else 0),
            -1,
            "ultralytics.nn.modules.block.C2f",
            3,
        )
        ...
        if number_heads > 1:
            ...
            layer16.i, layer16.f, layer16.type, layer16.n = (
                16 + (2 if deeper_head else 0) + (-1 if no_SPPF else 0),
                -1,
            ...
            layer17.i, layer17.f, layer17.type, layer17.n = (
                17 + (2 if deeper_head else 0) + (-1 if no_SPPF else 0),
            ...
            layer18.i, layer18.f, layer18.type, layer18.n = (
                18 + (2 if deeper_head else 0) + (-1 if no_SPPF else 0),
                -1,
                "ultralytics.nn.modules.block.C2f",
                3,
            )
            ...
            if number_heads > 2:
                ...
                layer19.i, layer19.f, layer19.type, layer19.n = (
                    19 + (2 if deeper_head else 0) + (-1 if no_SPPF else 0),
                    -1,
                    "ultralytics.nn.modules.conv.Conv",
                    1,
                )
        ...
        if task == "detect":
            ...
            head.i, head.f, head.type, head.n = (
                ...
                + 1
                + (2 if deeper_head else 0)
                + (-1 if no_SPPF else 0),
                head_concat_layers[:number_heads],
